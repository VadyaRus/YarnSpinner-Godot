...
namespace Yarn.GodotYarn {
    ...
    internal static class ActionManager {
        ...
        private static Injector GetInjectorForType(Type injectorType, ref Dictionary<string, Injector> injectorCache) {
            ...
            if (injectorCache.ContainsKey(fullyQualifiedName) == false) {
        ...
        private static DispatchCommand CreateCommandRunner(MethodInfo method, YarnCommandAttribute metadata, ref Dictionary<string, Injector> injectorCache) {
        ...
        private static void FindAllActions(IEnumerable<string> assemblyNames) {
            ...
            IEnumerable<Assembly> assemblies = AppDomain.CurrentDomain.GetAssemblies().Join(
                assemblyNamesToLoad,
                ...
                foreach (var attribute in attributes) {
                    if (attribute is YarnCommandAttribute command) {
                        ...
                        try {
                            ...
                            GD.PushError($"Can't add {method.DeclaringType.FullName}.{method.Name} for command {name} " +
                        ...
                        try {
                            ...
                            GD.PushError($"Can't add {method.DeclaringType.FullName}.{method.Name} for command {name}: {e.Message}");
                        }
                    }
                }
            }
        ...
        public static DialogueRunner.CommandDispatchResult TryExecuteCommand(string[] args, out object returnValue) {
            if (commands.TryGetValue(args[0], out var command) == false) {
                returnValue = null;
        ...
        public static object[] ParseArgs(MethodInfo method, Converter[] converters, string[] args, bool isStatic = false) {
            var parameters = method.GetParameters();
            ...
            if (optional > 0) {
                ...
                var requiredParameterTypeNames = string.Join(", ", parameters.Where(p => p.IsOptional == false).Select(p => p.ParameterType.ToString()));
